...
Pa = (eye(length(za_mean)) - K*H)*Pf;


%%%%% ET Step (Bishop 2001) %%%%%
%       U = zf_pert'*(H'*inv(R)*H + H_F'*inv(R_F)*H_F)*zf_pert/(N_ens - 1);
U = zf_pert'*(H'*inv(R)*H)*zf_pert/(N_ens - 1);
